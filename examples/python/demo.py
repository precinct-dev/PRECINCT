...
def unique_session_id(prefix: str) -> str:
    """Generate a deterministic, valid UUID v5 session ID.

    OC-3nnm requires session IDs to be valid UUIDs; non-UUID values are
    silently replaced by body_capture middleware, breaking cross-request
    session continuity needed for escalation detection tests.
    """
    ns = uuid.uuid5(uuid.NAMESPACE_DNS, f"precinct-demo-{DEMO_RUN_SUFFIX}")
