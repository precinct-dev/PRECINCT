...
func TestPrecinctIdentityIntegration_ListAndRegister(t *testing.T) {
	if err := waitForService(gatewayURL+"/health", 30*time.Second); err != nil {
		t.Fatalf("Gateway not ready: %v", err)
	}

	// List existing identities first.
	listCmd := exec.Command("go", "run", "./cli/precinct", "identity", "list", "--format", "json")
	listCmd.Dir = pocDir()
	var listOut, listErr bytes.Buffer
	listCmd.Stdout = &listOut
	listCmd.Stderr = &listErr
	if err := listCmd.Run(); err != nil {
		t.Fatalf("precinct identity list failed: %v stdout=%q stderr=%q", err, listOut.String(), listErr.String())
	}
	...
	name := fmt.Sprintf("precinct-int-%x", time.Now().UnixNano()&0xfffffff)
	selector := fmt.Sprintf("docker:label:spiffe-id:%s", name)

	registerCmd := exec.Command(
		"go", "run", "./cli/precinct", "identity", "register", name,
		"--confirm",
		"--selector", selector,
		"--format", "json",
	)
	...
	if err := registerCmd.Run(); err != nil {
		t.Fatalf("precinct identity register failed: %v stdout=%q stderr=%q", err, registerOut.String(), registerErr.String())
	}
	...
	verifyList := exec.Command("go", "run", "./cli/precinct", "identity", "list", "--format", "json")
	verifyList.Dir = pocDir()
	var verifyOut, verifyErr bytes.Buffer
	verifyList.Stdout = &verifyOut
	verifyList.Stderr = &verifyErr
	if err := verifyList.Run(); err != nil {
		t.Fatalf("precinct identity list verify failed: %v stdout=%q stderr=%q", err, verifyOut.String(), verifyErr.String())
	}
