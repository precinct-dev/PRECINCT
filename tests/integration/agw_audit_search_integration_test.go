...
func TestAgwAuditSearchIntegration_JSON(t *testing.T) {
	...
	if err := cmdByDecision.Run(); err != nil {
		t.Fatalf("agw audit search --decision-id failed: %v stdout=%q stderr=%q", err, stdout.String(), stderr.String())
	}
	...
	if err := cmdDenied.Run(); err != nil {
		t.Fatalf("agw audit search --denied failed: %v stdout=%q stderr=%q", err, stdout.String(), stderr.String())
	}
