...
// ---------------------------------------------------------------------------
// Inline mock OAuth issuer (HS256 JWT + JWKS)
// ---------------------------------------------------------------------------

// testOAuthIssuer is a minimal OAuth issuer that mints HS256 JWTs with
// deterministic signing keys. It exposes /health, /jwks.json, and /token
// endpoints, matching the contract of examples/mock-oauth-issuer.
...
func newTestOAuthIssuer(issuerURL, kid string) *testOAuthIssuer {
	...
	return &testOAuthIssuer{kid: kid, signingKey: key, issuerURL: issuerURL}
}
...
func (s *testOAuthIssuer) handleToken(w http.ResponseWriter, r *http.Request) {
	...
	jwt, err := s.signJWT(subject, audience, req.Scope, clientID, now, expiresAt)
	if err != nil {
