...
func TestPrecinctInspectRateLimitIntegration_JSON(t *testing.T) {
	...
	cmd := exec.Command("go", "run", "./cli/precinct", "inspect", "rate-limit", spiffeID, "--keydb-url", keydbURL, "--format", "json")
	cmd.Dir = pocDir()
	...
	if err := cmd.Run(); err != nil {
		...
		t.Fatalf("precinct inspect rate-limit failed: %v stdout=%q stderr=%q", err, stdout.String(), stderr.String())
	}
	...
	cmdAll := exec.Command("go", "run", "./cli/precinct", "inspect", "rate-limit", "--keydb-url", keydbURL, "--format", "json")
	cmdAll.Dir = pocDir()
	stdout.Reset()
	stderr.Reset()
	...
	if err := cmdAll.Run(); err != nil {
		t.Fatalf("precinct inspect rate-limit (all) failed: %v stdout=%q stderr=%q", err, stdout.String(), stderr.String())
	}
