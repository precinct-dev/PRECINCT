...
func TestAgwResetCircuitBreakerIntegration_OpenThenResetClosed(t *testing.T) {
	...
	stop := exec.Command("docker", "compose", "stop", "mock-mcp-server")
	stop.Dir = pocDir()
	if out, err := stop.CombinedOutput(); err != nil {
		t.Fatalf("docker compose stop mock-mcp-server failed: %v output=%q", err, string(out))
	}
	t.Cleanup(func() {
		up := exec.Command("docker", "compose", "up", "-d", "mock-mcp-server")
		up.Dir = pocDir()
	...
	opened := false
	for i := 0; i < 12; i++ {
		code := postGatewayMethod(t, spiffeID, tool, map[string]any{"query": "cb-reset-integration"})
	...
	if err := resetCmd.Run(); err != nil {
		t.Fatalf("agw reset circuit-breaker failed: %v stdout=%q stderr=%q", err, resetOut.String(), resetErr.String())
	}
	...
	if err := inspectClosed.Run(); err != nil {
		t.Fatalf("agw inspect circuit-breaker (closed) failed: %v stdout=%q stderr=%q", err, inspectClosedOut.String(), inspectClosedErr.String())
	}
