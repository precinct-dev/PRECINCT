...
func TestPrecinctStatusIntegration_JSONAndTable(t *testing.T) {
	...
	// JSON format
	{
		cmd := exec.Command("go", "run", "./cli/precinct", "status", "--gateway-url", gatewayURL, "--format", "json")
		cmd.Dir = pocDir()
	...
	// Table format
	{
		cmd := exec.Command("go", "run", "./cli/precinct", "status", "--gateway-url", gatewayURL, "--format", "table")
		cmd.Dir = pocDir()
	...
	{
		cmd := exec.Command("go", "run", "./cli/precinct", "status", "--component", "gateway", "--gateway-url", gatewayURL, "--format", "json")
		cmd.Dir = pocDir()
		out, err := cmd.CombinedOutput()
		if err != nil {
			t.Fatalf("precinct status --component gateway failed: %v\nOutput:\n%s", err, string(out))
		}
		var parsed struct {
			Components []struct {
				Name    string         `json:"name"`
