...
func TestAgwPolicyListIntegration_JSONAndFilteredTable(t *testing.T) {
	...
	if err := cmdJSON.Run(); err != nil {
		t.Fatalf("agw policy list json failed: %v stdout=%q stderr=%q", err, stdout.String(), stderr.String())
	}
	...
	if err := cmdFiltered.Run(); err != nil {
		t.Fatalf("agw policy list filtered table failed: %v stdout=%q stderr=%q", err, stdout.String(), stderr.String())
	}
	out := stdout.String()
