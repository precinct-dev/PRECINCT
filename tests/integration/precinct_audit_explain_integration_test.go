...
func TestPrecinctAuditExplainIntegration_DeniedRequest(t *testing.T) {
	...
	cmd := exec.Command("go", "run", "./cli/precinct", "audit", "explain", denied.DecisionID, "--format", "json")
	cmd.Dir = pocDir()
	var stdout, stderr bytes.Buffer
	cmd.Stdout = &stdout
	cmd.Stderr = &stderr
	if err := cmd.Run(); err != nil {
		t.Fatalf("precinct audit explain failed: %v stdout=%q stderr=%q", err, stdout.String(), stderr.String())
	}
