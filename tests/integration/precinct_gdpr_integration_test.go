...
func TestPrecinctGDPRDeleteIntegration_CreateDeleteVerifyLifecycle(t *testing.T) {
	...
	cmd := exec.Command(
		"go", "run", "./cli/precinct", "gdpr", "delete", spiffeID,
		"--confirm",
		"--source", "docker",
		"--project-root", ".",
	...
	if err := cmd.Run(); err != nil {
		t.Fatalf("precinct gdpr delete failed: %v stdout=%q stderr=%q", err, stdout.String(), stderr.String())
	}
...
func TestPrecinctGDPRAuditIntegration_ExportsCompleteDSARPackage(t *testing.T) {
	if err := waitForService(gatewayURL+"/health", 30*time.Second); err != nil {
		t.Fatalf("Gateway not ready: %v", err)
	}
	...
	outputDir := filepath.Join(t.TempDir(), "reports")
	cmd := exec.Command(
		"go", "run", "./cli/precinct", "gdpr", "audit", spiffeID,
		"--source", "docker",
		"--project-root", ".",
		"--output-dir", outputDir,
		"--keydb-url", keydbURL,
	...
	cmd.Stderr = &stderr
	if err := cmd.Run(); err != nil {
		t.Fatalf("precinct gdpr audit failed: %v stdout=%q stderr=%q", err, stdout.String(), stderr.String())
	}
