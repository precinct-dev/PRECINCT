...
func TestPrecinctResetRateLimitIntegration(t *testing.T) {
	...
	cmd := exec.Command("go", "run", "./cli/precinct", "reset", "rate-limit", spiffeID, "--confirm", "--keydb-url", keydbURL)
	cmd.Dir = pocDir()
	out, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("precinct reset rate-limit failed: %v\nOutput:\n%s", err, string(out))
	}
