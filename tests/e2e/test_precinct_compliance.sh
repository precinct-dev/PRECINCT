...
SESSION_ID="sid-precinct-compliance-$(date +%s)"
...
print_final_summary() {
  ...
  if [ "$FAIL_COUNT" -gt 0 ]; then
    echo -e "${RED}test_precinct_compliance.sh: FAIL${NC}"
    exit 1
  fi
  echo -e "${GREEN}test_precinct_compliance.sh: PASS${NC}"
}

main() {
  log_header "E2E precinct Compliance Validation"
  ...
  run_cmd "seed_audit_data" env PRECINCT_BIN="$PRECINCT_BIN" bash tests/e2e/test_precinct_cli.sh
