...
build_precinct_binary() {
  mkdir -p "$ROOT_DIR/build/bin"
  run_cmd "go_build_precinct" go build -o "$PRECINCT_BIN" ./cli/precinct/
}

generate_sdk_traffic() {
  log_header "Generate Live Traffic (Go SDK)"
  local helper_file="$TMP_DIR/precinct_cli_sdk_traffic.go"
...
func main() {
	...
	for i := 1; i <= 3; i++ {
		_, err := client.Call(ctx, "tavily_search", map[string]any{
			"query": fmt.Sprintf("precinct-cli-e2e-%d", i),
		})
		if err != nil {
	...
	_, err := client.Call(ctx, "tool_does_not_exist_for_precinct_demo", map[string]any{"reason": "audit explain demo"})
	if err == nil {
...
print_final_summary() {
  ...
  if [ "$FAIL_COUNT" -gt 0 ]; then
    echo -e "${RED}test_precinct_cli.sh: FAIL${NC}"
    exit 1
  fi

  echo -e "${GREEN}test_precinct_cli.sh: PASS${NC}"
}
