...
mission_purpose="$(jq -r '.adapter_contract.gateway_guardrails.mission_boundary.agent_purpose // ""' "${PACK_JSON}")"
mission_mode="$(jq -r '.adapter_contract.gateway_guardrails.mission_boundary.default_mode // ""' "${PACK_JSON}")"
mission_allowed_intents_count="$(jq -r '.adapter_contract.gateway_guardrails.mission_boundary.allowed_intents | length' "${PACK_JSON}")"
mission_allowed_topics_count="$(jq -r '.adapter_contract.gateway_guardrails.mission_boundary.allowed_topics | length' "${PACK_JSON}")"
mission_blocked_topics_count="$(jq -r '.adapter_contract.gateway_guardrails.mission_boundary.blocked_topics | length' "${PACK_JSON}")"
mission_out_of_scope_action="$(jq -r '.adapter_contract.gateway_guardrails.mission_boundary.out_of_scope_action // ""' "${PACK_JSON}")"
...
[[ -n "${mission_purpose}" ]] || fail "adapter_contract.gateway_guardrails.mission_boundary.agent_purpose must be set"
[[ "${mission_mode}" == "enforce" || "${mission_mode}" == "advisory" ]] || fail "adapter_contract.gateway_guardrails.mission_boundary.default_mode must be 'enforce' or 'advisory'"
[[ "${mission_allowed_intents_count}" -ge 1 ]] || fail "adapter_contract.gateway_guardrails.mission_boundary.allowed_intents must include at least one value"
[[ "${mission_allowed_topics_count}" -ge 1 ]] || fail "adapter_contract.gateway_guardrails.mission_boundary.allowed_topics must include at least one value"
[[ "${mission_blocked_topics_count}" -ge 1 ]] || fail "adapter_contract.gateway_guardrails.mission_boundary.blocked_topics must include at least one value"
[[ "${mission_out_of_scope_action}" == "deny" || "${mission_out_of_scope_action}" == "rewrite" || "${mission_out_of_scope_action}" == "handoff" ]] || fail "adapter_contract.gateway_guardrails.mission_boundary.out_of_scope_action must be deny/rewrite/handoff"
...
pass "adapter contract sections present (model routing, tool registration, mission boundary, guardrails, runtime profile hints)"
