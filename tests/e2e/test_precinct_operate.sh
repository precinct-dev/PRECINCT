...
SESSION_ID="sid-precinct-operate-$(date +%s)"
RUNTIME_SESSION_ID="sid-precinct-runtime-$(date +%s)"
DENIED_DECISION_ID=""
...
print_final_summary() {
  ...
  if [ "$FAIL_COUNT" -gt 0 ]; then
    echo -e "${RED}test_precinct_operate.sh: FAIL${NC}"
    exit 1
  fi
  echo -e "${GREEN}test_precinct_operate.sh: PASS${NC}"
}
...
main() {
  ...
  gateway_call "deny_unknown_tool" "tool_does_not_exist_for_precinct_operate_demo" '{"reason":"audit-seed"}' "$SESSION_ID"
...
PY
  ...
  gateway_call "rate_limited_before_reset" "$TOOL_NAME" '{"query":"precinct rate limit before reset"}' "$SESSION_ID"
  if [ "$GATEWAY_CODE" != "429" ]; then
  ...
  gateway_call "rate_limit_after_reset" "$TOOL_NAME" '{"query":"precinct rate limit after reset"}' "$SESSION_ID"
  if [ "$GATEWAY_CODE" = "429" ]; then
