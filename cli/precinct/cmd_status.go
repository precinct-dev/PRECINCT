...
func newStatusCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			out, allOK, err := precinctcli.CollectStatus(ctx, cfg, precinctcli.DefaultDeps())
			if err != nil {
				return err
			}
			...
			switch format {
			case "", "table":
				s, err := precinctcli.RenderTable(out)
				if err != nil {
					return fmt.Errorf("render table: %w", err)
				}
				_, _ = cmd.OutOrStdout().Write([]byte(s))
			case "json":
				b, err := precinctcli.RenderJSON(out)
				if err != nil {
