...
type spikeManager interface {
	ListSecretRefs(ctx context.Context) ([]precinctcli.SPIKESecretRef, error)
	PutSecret(ctx context.Context, ref, value string) (precinctcli.SPIKESecretPutResult, error)
}

var newSPIKEManager = func() spikeManager {
	return precinctcli.NewSPIKECLI()
}
...
func newSecretListCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			switch format {
			case "", "table":
				s, err := precinctcli.RenderSecretListTable(refs)
				if err != nil {
					return fmt.Errorf("render table: %w", err)
				}
				_, _ = cmd.OutOrStdout().Write([]byte(s))
			case "json":
				b, err := precinctcli.RenderSecretListJSON(refs)
				if err != nil {
...
func newSecretPutCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			switch format {
			case "", "table":
				s, err := precinctcli.RenderSecretPutTable(result)
				if err != nil {
					return fmt.Errorf("render table: %w", err)
				}
				_, _ = cmd.OutOrStdout().Write([]byte(s))
			case "json":
				b, err := precinctcli.RenderSecretPutJSON(result)
				if err != nil {
					return fmt.Errorf("render json: %w", err)
				}
				_, _ = cmd.OutOrStdout().Write(b)
