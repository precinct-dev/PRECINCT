...
func TestPrecinctGDPRAudit_JSONOutput(t *testing.T) {
	orig := gdprExportDSARPackage
	t.Cleanup(func() { gdprExportDSARPackage = orig })

	gdprExportDSARPackage = func(_ctx context.Context, p precinctcli.GDPRAuditParams) (precinctcli.DSARExportResult, error) {
		if p.SPIFFEID != "spiffe://poc.local/agents/example/dev" {
			t.Fatalf("unexpected params: %+v", p)
		}
		return precinctcli.DSARExportResult{
			SPIFFEID:            p.SPIFFEID,
	...
	var parsed precinctcli.DSARExportResult
