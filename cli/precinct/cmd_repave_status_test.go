...
func TestPrecinctRepaveStatus_JSON(t *testing.T) {
	orig := collectRepaveStatus
	t.Cleanup(func() { collectRepaveStatus = orig })

	collectRepaveStatus = func(_ctx context.Context, p precinctcli.RepaveStatusParams) (precinctcli.RepaveStatusOutput, error) {
		if p.StateFile != ".repave-state.json" {
			t.Fatalf("unexpected state file: %+v", p)
		}
		return precinctcli.RepaveStatusOutput{
			Containers: []precinctcli.RepaveContainerStatus{
				{
					Name:       "keydb",
...
func TestPrecinctRepaveStatus_TableDefault(t *testing.T) {
	orig := collectRepaveStatus
	t.Cleanup(func() { collectRepaveStatus = orig })
