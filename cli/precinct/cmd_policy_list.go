...
func newPolicyListCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			out, err := precinctcli.ListPolicyGrants(opaPolicyDir, toolRegistryPath, spiffeID)
			if err != nil {
				return err
			}
			...
			switch format {
			case "", "table":
				s, err := precinctcli.RenderPolicyListTable(out)
				if err != nil {
					return fmt.Errorf("render table: %w", err)
				}
				_, _ = cmd.OutOrStdout().Write([]byte(s))
			case "json":
				b, err := precinctcli.RenderPolicyListJSON(out)
				if err != nil {
