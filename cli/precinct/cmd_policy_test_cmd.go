	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if runtime {
				...
				out, err = precinctcli.RunPolicyTestOffline(spiffeID, tool, params, opaPolicyDir, toolRegistryPath)
			}
			if err != nil {
				return err
			}
			...
			switch format {
			case "", "table":
				s, err := precinctcli.RenderPolicyTestOfflineTable(out)
				if err != nil {
					return fmt.Errorf("render table: %w", err)
				}
				_, _ = cmd.OutOrStdout().Write([]byte(s))
			case "json":
				b, err := precinctcli.RenderPolicyTestOfflineJSON(out)
				if err != nil {
