...
**Go dependencies** (batch by group):

| Group | Modules | Target | Notes |
|-------|---------|--------|-------|
| OpenTelemetry | `otel`, `otel/trace`, `otel/metric`, `otel/sdk`, `otlptracegrpc`, `otlpmetricgrpc` | 1.42.0 | All 6 must update together; may require Go >= 1.25 |
| AWS SDK | `aws-sdk-go-v2`, `config`, `service/s3` | patch releases | No breaking changes |
| Redis | `github.com/redis/go-redis` | 9.18.0 | Additive; includes PubSub nil pointer fix |

CI workflows are currently disabled (moved to `.github/workflows-disabled/`). Re-enable by moving them back once rename and dependency updates are complete.
