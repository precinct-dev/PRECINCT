...
### Why OpenClaw Is a Useful Stress Test

OpenClaw is designed to be powerful and convenient, which means it has many untrusted ingress paths (Telegram, WhatsApp, Slack, Discord, Signal, and others), many direct egress paths (model provider calls, web fetch/search, media downloaders, embeddings APIs), and secrets as the happy-path configuration style. It also includes operator-selectable modes that can degrade security posture by configuration drift -- for example, sandboxing is opt-in in several paths and exec approvals can be broadly expanded. This makes it an honest test of the architecture: if PRECINCT can constrain OpenClaw to an enterprise-defensible posture without touching its source, the same pattern should apply to any comparable agent framework.
...
### Multi-Channel Ingress: The Channel Connector Pattern

Applications like OpenClaw add many "front doors" -- Telegram, WhatsApp, Slack, Discord, and others. Each is a distinct untrusted ingress path with its own secrets (bot tokens, app tokens) and outbound exfiltration routes. The recommended enterprise pattern is:

- Deploy **Channel Connectors** as managed services adjacent to the tool plane, not as part of the agent runtime. Each connector speaks one channel's native protocol (Telegram Bot API, WhatsApp Business API, etc.), converts inbound events into the PRECINCT Gateway ingress envelope, and submits them to Ingress Admission before any content can influence session state, prompts, or tool calls.
- Outbound channel responses should be exposed as governed tools (e.g., `send_message(channel=telegram, ...)`) executed via the tool plane, not called directly from the agent runtime.
- This keeps PRECINCT Gateway protocol-agnostic while providing consistent, centralized security enforcement across every channel.

This pattern works in Docker Compose: split the agent runtime into a container with no Internet egress and expose only the PRECINCT Gateway and connector networks. In Kubernetes, enforce with NetworkPolicies and optionally a service mesh egress policy.

### DLP Rules Must Be Vendor-Neutral

Agent frameworks often support provider-specific key formats beyond the common `sk-` prefix -- for example, `ZAI_API_KEY`, `GROQ_API_KEY`, and enterprise-specific credential patterns. DLP rules must therefore be:

- Configurable via RuleOps CRUD (not hardcoded pattern lists)
- Reviewable, promotable, and version-pinned with active digests for audit
- Vendor-neutral in pattern coverage -- do not assume a finite list of known key prefixes

### Residual Gaps After Adaptation

Even after full gateway adaptation, some areas remain operational and governance-dependent rather than technically enforceable:

- Channel onboarding approvals (who can talk to the agent, and under what conditions)
- Budget governance and exception handling processes
...
The architecture provides scaffolding -- policy hooks, audit surfaces, CLI tooling -- but organizations must supply their own governance processes for these areas.

### Skill Supply-Chain Risk

Recent research (Snyk ToxicSkills study, February 2026) auditing 3,984 AI agent skills found that 36.82% contain at least one security flaw, 13.4% have a critical-level issue, and 91% of confirmed malicious skills simultaneously employ prompt injection alongside malware. The academic literature (arXiv:2601.17548) reports prompt injection attacks achieve 85%+ success rates against state-of-the-art defenses.

The correct defense is a **pre-registration gate** that operates upstream of the tool registry, not at gateway runtime. Skills must be scanned and signed before their tools enter the signed tool registry that the gateway trusts. Running a scan step inside the gateway middleware chain would add latency, violate the middleware chain design, and introduce a category confusion between supply-chain verification (a registration-time concern) and runtime request processing.

The recommended architecture:
1. Download skills and run offline static analysis (prompt injection, malicious code patterns, credential leakage) using tools such as skulto (`skulto scan --format json`).
2. Apply additional analysis: existing `containsPoisoningPattern()` logic against tool descriptions, SHA-256 hash computation, DLP pattern checks.
3. If checks pass, generate tool registry YAML entries and sign with `cosign sign-blob`.
4. The gateway's existing registry hot-reload mechanism (ADR-006) picks up the signed update automatically.

This preserves the gateway's security invariants (no new runtime dependencies) while adding skill-level supply-chain verification. Even after pre-registration scanning, the gateway still enforces hash verification (step 5), poisoning pattern detection (step 5), DLP scanning (step 7), deep scan (step 10), OPA policy (step 6), and token substitution (step 13) as defense in depth.

Note: mcp-scan (Invariant Labs) sends tool descriptions to an external API, which conflicts with air-gapped requirements and introduces a third-party data flow requiring GDPR assessment. Prefer offline-first scanning tools for regulated environments.
