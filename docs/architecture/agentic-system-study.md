...
| AGE Go driver | `<local-clone>/allage/age/drivers/golang/` (AGE Go driver source) | Vendor or publish as Go module |
...
|---------|-------|--------|
| rlm-sidecar (Python) | `rlm.RLM().completion()` | `<local-clone>/rlm/core/rlm.py` |
| rustreason-sidecar (Rust) | `rustreason_core::ReasoningEngine` | `<local-clone>/crates/rustreason-core/` |
| synareason-sidecar (Python) | `synareason.Session` | `<local-clone>/src/` |
