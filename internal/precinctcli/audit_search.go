...
func loadAuditFromDockerComposeLogs(ctx context.Context, projectRoot string) ([]map[string]any, error) {
	for _, auditPath := range []string{"/tmp/audit.jsonl", "/var/log/gateway/audit.jsonl"} {
		content, err := copyComposeServiceFile(ctx, "precinct-gateway", auditPath)
		if err == nil {
			return parseAuditJSONLLines(bytes.NewReader(content))
		}
	}
	...
	cmd := exec.CommandContext(ctx, "docker", composeArgs("logs", "--no-log-prefix", "precinct-gateway")...)
	cmd.Dir = root
