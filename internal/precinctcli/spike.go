...
func spikeListPrimaryArgs() []string {
	...
	return append([]string{"docker"}, composeArgs("exec", "-T", "spike-nexus", "spike", "pilot", "list")...)
}

func spikeListFallbackArgs() []string {
	// The currently running stack exposes the CLI through the spike-pilot image.
	return append([]string{"docker"}, composeArgs("run", "--rm", "--no-deps", "--entrypoint", "/usr/local/bin/spike", "spike-secret-seeder", "secret", "list")...)
}

func spikePutPrimaryArgs(ref, value string) []string {
	// Story-required command path:
	// docker compose exec spike-nexus spike pilot put --ref <ref> --value <value>
	return append([]string{"docker"}, composeArgs("exec", "-T", "spike-nexus", "spike", "pilot", "put", "--ref", ref, "--value", value)...)
}

func spikePutFallbackArgs(ref, value string) []string {
	return append([]string{"docker"}, composeArgs("run", "--rm", "--no-deps", "--entrypoint", "/usr/local/bin/spike", "spike-secret-seeder", "secret", "put", ref, "value="+value)...)
}
