...
// ---------------------------------------------------------------------------
// rlmLimits defines the per-lineage enforcement bounds for nested agent calls.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// rlmLineageState tracks the cumulative resource usage for a single lineage.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// rlmExecutionInput captures the parsed and validated fields from an inbound
// PlaneRequestV2 that are relevant to RLM governance evaluation.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// rlmGovernanceEngine is the stateful core of RLM governance. It maintains
// per-lineage state behind a mutex and evaluates each inbound request against
// depth, subcall, and budget limits.
// ---------------------------------------------------------------------------
...
// evaluate processes an RLM governance request. It returns:
//   - handled:  whether the engine engaged (false means the request is not RLM)
//   - decision: allow/deny
//   - reason:   machine-readable reason code
//   - status:   HTTP status code
//   - meta:     metadata map with lineage/budget details
func (e *rlmGovernanceEngine) evaluate(req PlaneRequestV2) (bool, Decision, ReasonCode, int, map[string]any) {
	...
	// Subcall without UASGS mediation is an attempted bypass.
	...
	// Enforce limits before consuming resources.
...
// metadata builds the response metadata map that accompanies every RLM decision.
...
func parseRLMExecutionInput(req PlaneRequestV2) (rlmExecutionInput, error) {
	...
	// Not an RLM request at all.
	if mode != "rlm" && !subcall {
		return rlmExecutionInput{Apply: false}, nil
	}
	// rlm_subcall=true but wrong mode is a schema error.
	if mode != "rlm" && subcall {
...
// ---------------------------------------------------------------------------
// Numeric utilities (NOT redefining getFloatAttr -- it lives in
// phase3_plane_stubs.go).
// ---------------------------------------------------------------------------
