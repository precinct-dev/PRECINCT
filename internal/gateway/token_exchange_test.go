...
func TestCredentialLookupTimingSafe(t *testing.T) {
	// Verify that credential type comparison uses constant-time comparison
	// by testing that LookupCredential correctly handles matching and
	// non-matching types, including types that share common prefixes.
	cfg := LoadTokenExchangeConfigForTest(
		[]TokenExchangeCredential{
			{
				CredentialType: "api_key",
				CredentialHash: hashCredential(t, "secret-001"),
				SPIFFEID:       "spiffe://poc.local/external/tool-a",
			},
			{
				CredentialType: "bearer_token",
				CredentialHash: hashCredential(t, "secret-002"),
				SPIFFEID:       "spiffe://poc.local/external/tool-b",
			},
		},
		testSigningKey,
		15*time.Minute,
	...
	t.Run("exact type match succeeds", func(t *testing.T) {
		spiffeID, ok := cfg.LookupCredential("api_key", "secret-001")
		if !ok {
			t.Fatal("expected credential to be found for exact type match")
		}
		if spiffeID != "spiffe://poc.local/external/tool-a" {
			t.Errorf("spiffe_id = %q, want spiffe://poc.local/external/tool-a", spiffeID)
		}
	})

	t.Run("prefix of valid type does not match", func(t *testing.T) {
		_, ok := cfg.LookupCredential("api_ke", "secret-001")
		if ok {
			t.Fatal("prefix of valid type should not match (constant-time compare is length-sensitive)")
		}
	})

	t.Run("type with extra suffix does not match", func(t *testing.T) {
		_, ok := cfg.LookupCredential("api_keys", "secret-001")
		if ok {
			t.Fatal("type with extra suffix should not match")
		}
	})

	t.Run("empty type does not match", func(t *testing.T) {
		_, ok := cfg.LookupCredential("", "secret-001")
		if ok {
			t.Fatal("empty type should not match")
		}
	})

	t.Run("correct type with wrong credential returns false", func(t *testing.T) {
		_, ok := cfg.LookupCredential("api_key", "wrong-secret")
		if ok {
			t.Fatal("wrong credential should not match even with correct type")
		}
	})

	t.Run("credential matches only its own type", func(t *testing.T) {
		// secret-002 is registered under bearer_token, not api_key
		_, ok := cfg.LookupCredential("api_key", "secret-002")
		if ok {
			t.Fatal("credential should not match under wrong type")
		}

		// But it should match under bearer_token
		spiffeID, ok := cfg.LookupCredential("bearer_token", "secret-002")
		if !ok {
			t.Fatal("credential should match under its own type")
		}
		if spiffeID != "spiffe://poc.local/external/tool-b" {
			t.Errorf("spiffe_id = %q, want spiffe://poc.local/external/tool-b", spiffeID)
		}
	})
}
