...
func mustNewApprovalSvc(t *testing.T) *middleware.ApprovalCapabilityService {
	t.Helper()
	svc, err := middleware.NewApprovalCapabilityService("test-key", 5*time.Minute, 30*time.Minute, nil, "dev")
	if err != nil {
...
func TestAdminApprovalsLifecycle(t *testing.T) {
	g := &Gateway{
		approvalCapabilities: mustNewApprovalSvc(t),
	}
