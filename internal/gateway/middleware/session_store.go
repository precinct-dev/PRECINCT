...
// It is intended for development and testing only. For production, use KeyDBStore.
// Safe for concurrent use via internal mutex. Sessions are automatically evicted
// after the configured TTL via both a periodic cleanup goroutine and inline
...
// NewInMemoryStore creates a new InMemoryStore with a default 1-hour session TTL.
func NewInMemoryStore() *InMemoryStore {
	return NewInMemoryStoreWithTTL(time.Hour)
}

// NewInMemoryStoreWithTTL creates a new InMemoryStore with the specified session TTL.
// A background goroutine periodically evicts expired sessions. Call Close() to stop it.
...
func (s *InMemoryStore) evictExpired() {
	...
	defer s.mu.Unlock()
	now := time.Now()
	for key, created := range s.createdAt {
...
func (s *InMemoryStore) GetSession(_ context.Context, spiffeID, sessionID string) (*AgentSession, error) {
	...
	created, hasTime := s.createdAt[key]
	s.mu.RUnlock()
	if !exists {
		return nil, nil
	}
	// Belt-and-suspenders: inline expiration check in addition to background cleanup.
	if hasTime && time.Since(created) > s.ttl {
		s.mu.Lock()
		delete(s.sessions, key)
		delete(s.createdAt, key)
		s.mu.Unlock()
...
func (s *InMemoryStore) SaveSession(_ context.Context, spiffeID, sessionID string, session *AgentSession) error {
	...
	if _, exists := s.createdAt[key]; !exists {
