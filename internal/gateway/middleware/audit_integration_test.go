...
// TestAuditLogFilePermissions verifies that audit log files are created with
// 0600 permissions and audit directories with 0700 permissions, preventing
// unauthorized read access by other system users.
func TestAuditLogFilePermissions(t *testing.T) {
	// Setup: use a subdirectory inside TempDir so we can verify the
	// directory permissions set by MkdirAll (TempDir itself is created
	// by the test framework, not by our code).
	tmpDir := t.TempDir()
	auditDir := filepath.Join(tmpDir, "audit_logs")
	auditPath := filepath.Join(auditDir, "permissions_test.jsonl")
	...
	if err := os.WriteFile(registryPath, []byte("tools:\n  - file_read"), 0644); err != nil {
		t.Fatalf("Failed to create registry file: %v", err)
	}

	// Create auditor -- this should create the directory and file
	auditor, err := NewAuditor(auditPath, bundlePath, registryPath)
	if err != nil {
		t.Fatalf("Failed to create auditor: %v", err)
	}

	// Log one event so the file has content
	auditor.Log(AuditEvent{
		SessionID:  "perm-test",
		DecisionID: "perm-decision",
		TraceID:    "perm-trace",
	...
	// AC1: Verify audit log file permissions are 0600
	fileInfo, err := os.Stat(auditPath)
	if err != nil {
		t.Fatalf("Failed to stat audit file: %v", err)
	}
	filePerm := fileInfo.Mode().Perm()
	if filePerm != 0600 {
		t.Errorf("AC1 FAILED: Audit file permissions = %o, want 0600", filePerm)
	} else {
		t.Log("AC1 PASSED: Audit file permissions are 0600")
	}

	// AC2: Verify audit directory permissions are 0700
	dirInfo, err := os.Stat(auditDir)
	if err != nil {
		t.Fatalf("Failed to stat audit directory: %v", err)
	}
	dirPerm := dirInfo.Mode().Perm()
	if dirPerm != 0700 {
		t.Errorf("AC2 FAILED: Audit directory permissions = %o, want 0700", dirPerm)
	} else {
		t.Log("AC2 PASSED: Audit directory permissions are 0700")
	}
}
