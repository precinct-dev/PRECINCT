...
func ValidateOAuthJWT(ctx context.Context, rawToken string, cfg OAuthJWTConfig) (OAuthJWTClaims, error) {
	...
	// Only asymmetric algorithms are allowed when validating against a JWKS
	// endpoint. Accepting HMAC (HS256/HS384/HS512) here would enable
	// CVE-2015-9235 algorithm-confusion attacks: an adversary could sign a
	// forged token with HS256 using the (public) JWKS key bytes as the
	// symmetric secret.
