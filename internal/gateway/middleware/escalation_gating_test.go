...
func TestEscalationDetection_DemoScenario(t *testing.T) {
	...
	// OC-3nnm: SessionContextMiddleware now binds session IDs to SPIFFE identity
	// via HMAC, so we must use the bound session ID to look up the session.
	boundSessionID := BindSessionToIdentity(spiffeID, sessionID)
	session := sc.GetOrCreateSession(spiffeID, boundSessionID)
