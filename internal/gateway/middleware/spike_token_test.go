...
func TestValidateTokenExpiry(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "AcceptsValidIssuedAtWithExp",
			token: &SPIKEToken{
				Ref:      "abc123",
				Exp:      7200,
				IssuedAt: now - 100, // Issued 100s ago, expires in 7100s
			},
			wantError: false,
		},
	}
...
// TestValidateTokenExpiry_RejectsZeroIssuedAtWithNonZeroExp verifies that
// tokens with a relative expiry but no IssuedAt are rejected (fail-closed).
// Previously, ValidateTokenExpiry auto-populated IssuedAt with time.Now(),
// making such tokens effectively immortal.
func TestValidateTokenExpiry_RejectsZeroIssuedAtWithNonZeroExp(t *testing.T) {
	token := &SPIKEToken{
		Ref:      "abc123",
		Exp:      3600,
		IssuedAt: 0, // Zero: no IssuedAt set
	}

	err := ValidateTokenExpiry(token)
	if err == nil {
		t.Fatal("ValidateTokenExpiry() should reject token with Exp>0 and IssuedAt==0")
	}
	if !errors.Is(err, ErrTokenExpired) {
		t.Errorf("ValidateTokenExpiry() error = %v, want wrapped ErrTokenExpired", err)
	}

	// Verify IssuedAt was NOT mutated (the old bug would set it to time.Now())
	if token.IssuedAt != 0 {
		t.Errorf("Token IssuedAt was mutated to %d, should remain 0", token.IssuedAt)
	}
}

// TestValidateTokenExpiry_AcceptsZeroExpRegardlessOfIssuedAt verifies that
// tokens with no expiry (Exp==0) are always accepted, regardless of IssuedAt.
func TestValidateTokenExpiry_AcceptsZeroExpRegardlessOfIssuedAt(t *testing.T) {
	tests := []struct {
		name  string
		token *SPIKEToken
	}{
		{
			name: "Exp=0 IssuedAt=0",
			token: &SPIKEToken{
				Ref:      "abc123",
				Exp:      0,
				IssuedAt: 0,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := ValidateTokenExpiry(tt.token)
			if err != nil {
				t.Errorf("ValidateTokenExpiry() should accept Exp=0 regardless of IssuedAt, got error: %v", err)
			}
		})
	}
}
