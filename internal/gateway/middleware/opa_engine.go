...
// ErrOPAAttestationRequiredInProd is returned when a production enforcement profile
// is active but no OPA policy reload public key has been configured.
// OC-suzg: Unsigned hot-reloads are not allowed in production.
var ErrOPAAttestationRequiredInProd = errors.New("OPA policy attestation public key is required in production profiles")
...
type OPAEngineConfig struct {
	...
	// EnforcementProfile controls whether attestation is mandatory.
	// When set to a production profile (prod, prod_standard, prod_regulated_hipaa),
	// policy hot-reload is rejected unless a valid public key is configured.
	// OC-suzg: Make attestation mandatory in prod profiles.
...
type OPAEngine struct {
	...
	enforcementProfile    string // OC-suzg: cached for attestation enforcement checks
...
func NewOPAEngine(policyDir string, cfg ...OPAEngineConfig) (*OPAEngine, error) {
	...
	engine := &OPAEngine{
		...
		enforcementProfile: runtimeCfg.EnforcementProfile,
...
func (e *OPAEngine) Reload() (OPAEngineReloadResult, error) {
	...
	} else if isProductionProfile(e.enforcementProfile) {
		// OC-suzg: Production profiles require attestation for hot-reload.
		// Reject unsigned reloads rather than silently accepting them.
		result.Rejected = true
		result.RejectionReason = fmt.Sprintf("attestation required in production profile %q but no public key configured", e.enforcementProfile)
		return result, fmt.Errorf("%w (profile=%s)", ErrOPAAttestationRequiredInProd, e.enforcementProfile)
	}
...
func (e *OPAEngine) startWatcher() error {
	...
	if e.HasPublicKey() {
		...
		// OC-suzg: Production profiles MUST have attestation configured.
		// Reject startup rather than allowing unsigned hot-reloads.
		_ = watcher.Close()
		return fmt.Errorf("%w (profile=%s)", ErrOPAAttestationRequiredInProd, e.enforcementProfile)
	} else {
