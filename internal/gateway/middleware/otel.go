...
// It produces spans under the "precinct-gateway" instrumentation scope.
...
var tracer = otel.Tracer("precinct-gateway", trace.WithInstrumentationVersion("2.0.0"))

// meter is the package-level OTel meter used by all middleware in the gateway.
// It produces metrics under the "precinct-gateway" instrumentation scope.
// When no MeterProvider is registered, otel.Meter returns a no-op meter.
var meter = otel.Meter("precinct-gateway")
