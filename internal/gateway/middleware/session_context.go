...
// BindSessionToIdentity derives a deterministic, identity-bound session ID from the
// caller-provided session ID and the SPIFFE identity using HMAC-SHA256. This ensures
// that two different SPIFFE identities providing the same caller session ID will
// receive completely different internal session IDs, preventing session hijacking.
//
// OC-3nnm: The SPIFFE ID is used as the HMAC key and the caller session ID as the
// message. The output is a 64-character hex string that is deterministic for the
// same (spiffeID, callerSessionID) pair but unpredictable without knowing the
// SPIFFE identity.
func BindSessionToIdentity(spiffeID, callerSessionID string) string {
	mac := hmac.New(sha256.New, []byte(spiffeID))
	mac.Write([]byte(callerSessionID))
...
func SessionContextMiddleware(next http.Handler, sessionCtx *SessionContext) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// OC-3nnm: Bind session ID to SPIFFE identity using HMAC-SHA256.
		// This prevents session hijacking by ensuring that the same caller-
		// provided session ID maps to different internal session IDs for
		// different SPIFFE identities. Even if agent B knows agent A's
		// session ID, BindSessionToIdentity(B_spiffe, sessionID) produces
		// a completely different key than BindSessionToIdentity(A_spiffe, sessionID).
		boundSessionID := sessionID
		if spiffeID != "" {
			boundSessionID = BindSessionToIdentity(spiffeID, sessionID)
			if boundSessionID != sessionID {
				slog.Debug("session ID bound to SPIFFE identity",
		...
		// Get or create session using the identity-bound session ID
		session := sessionCtx.GetOrCreateSession(spiffeID, boundSessionID)
