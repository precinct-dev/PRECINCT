...
func TokenSubstitution(next http.Handler, redeemer SecretRedeemer, auditor *Auditor, scopeResolver ScopeResolver) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		for _, tokenStr := range tokenStrings {
			...
			// The redeemer populates token.OwnerID and token.IssuedAt from
			// SPIKE Nexus metadata (or from POC simulation). This must happen
			// BEFORE ownership and expiry validation so the gateway has the
			// server-assigned metadata to verify against. See RFA-7ct.
			...
			// Validate token expiry (after redemption, which sets IssuedAt).
			// Tokens with relative expiry (Exp > 0) require IssuedAt to be
			// set by the redeemer. Fail closed if IssuedAt is missing.
