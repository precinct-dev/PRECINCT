...
// mustStepUpNewApprovalSvc is a test helper for step-up gating tests.
func mustStepUpNewApprovalSvc(t *testing.T, signingKey string) *ApprovalCapabilityService {
	t.Helper()
	svc, err := NewApprovalCapabilityService(signingKey, 5*time.Minute, 30*time.Minute, nil, "dev")
	if err != nil {
...
func TestStepUpGating_ApprovalToken_AllowsCriticalTool(t *testing.T) {
	...
	svc := mustStepUpNewApprovalSvc(t, "test-key")
