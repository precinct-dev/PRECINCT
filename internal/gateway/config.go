...
type Config struct {
	...
	// EnableTokenExchangePublic, when true, automatically appends
	// /v1/auth/token-exchange to the public route allowlist so operators
	// do not need to modify PUBLIC_ROUTE_ALLOWLIST directly. Opt-in only.
	// OC-7z5q: Dedicated enable flag for token exchange public allowlist.
	EnableTokenExchangePublic bool
}
...
func ConfigFromEnv() *Config {
	...
	// OC-7z5q: Dedicated enable flag for token exchange on the public listener.
	enableTokenExchangePublic := parseEnvBool("ENABLE_TOKEN_EXCHANGE_PUBLIC", false)
	publicRouteAllowlist := getEnvOrDefault("PUBLIC_ROUTE_ALLOWLIST", defaultPublicRouteAllowlist)
	if enableTokenExchangePublic {
		slog.Warn("Token exchange endpoint exposed on public listener")
		// Append the token exchange route if not already present.
		parsed := parsePublicRouteAllowlist(publicRouteAllowlist)
		if _, already := parsed["/v1/auth/token-exchange"]; !already {
			publicRouteAllowlist = publicRouteAllowlist + ",/v1/auth/token-exchange"
		}
	}
	...
	return &Config{
		...
		PublicRouteAllowlist:            publicRouteAllowlist,
		...
		EnableTokenExchangePublic:       enableTokenExchangePublic,
	}
}
