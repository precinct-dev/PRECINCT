...
func TestAgwGDPRDelete_RequiresConfirm(t *testing.T) {
...
func TestAgwGDPRDelete_JSONOutput(t *testing.T) {
	orig := gdprDeleteSubjectData
	t.Cleanup(func() { gdprDeleteSubjectData = orig })

	gdprDeleteSubjectData = func(_ctx context.Context, p agw.GDPRDeleteParams) (agw.GDPRDeleteReport, error) {
		if p.SPIFFEID != "spiffe://poc.local/agents/example/dev" {
			t.Fatalf("unexpected params: %+v", p)
		}
		return agw.GDPRDeleteReport{
			SPIFFEID:  p.SPIFFEID,
			Timestamp: "2026-02-11T10:00:00Z",
			Categories: []agw.GDPRDeleteCategory{
	...
	var parsed agw.GDPRDeleteReport
