...
func TestAgwResetCircuitBreaker_AllWithoutConfirm_Fails(t *testing.T) {
...
func TestAgwResetCircuitBreaker_PromptNo_Aborts(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := run(
...
func TestAgwResetCircuitBreaker_PromptYes_Table(t *testing.T) {
	var gotTool string
...
func TestAgwResetCircuitBreaker_AllConfirm_JSON(t *testing.T) {
	var gotTool string
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost || r.URL.Path != "/admin/circuit-breakers/reset" {
			w.WriteHeader(http.StatusNotFound)
