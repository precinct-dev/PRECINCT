...
func TestAgwInspectCircuitBreaker_JSON_OK(t *testing.T) {
...
func TestAgwInspectCircuitBreaker_SpecificTool(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
...
func TestAgwInspectCircuitBreaker_Table_Default(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"circuit_breakers":[{"tool":"bash","state":"closed","failures":0,"threshold":5,"reset_timeout_seconds":30,"last_state_change":null}]}`))
	}))
