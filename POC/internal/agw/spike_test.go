...
func TestSPIKECLI_ListSecretRefs_Fallback(t *testing.T) {
	...
	wantFirst := []string{"docker", "compose", "exec", "-T", "spike-nexus", "spike", "pilot", "list"}
	...
	wantSecond := []string{"docker", "compose", "run", "--rm", "--no-deps", "--entrypoint", "/usr/local/bin/spike", "spike-secret-seeder", "secret", "list"}
...
func TestSPIKECLI_PutSecret_Fallback(t *testing.T) {
	...
	wantFirst := []string{"docker", "compose", "exec", "-T", "spike-nexus", "spike", "pilot", "put", "--ref", "deadbeef", "--value", "super-secret"}
	if !reflect.DeepEqual(runner.calls[0], wantFirst) {
		t.Fatalf("unexpected first put command: got=%v want=%v", runner.calls[0], wantFirst)
	}
	wantSecond := []string{"docker", "compose", "run", "--rm", "--no-deps", "--entrypoint", "/usr/local/bin/spike", "spike-secret-seeder", "secret", "put", "deadbeef", "value=super-secret"}
	if !reflect.DeepEqual(runner.calls[1], wantSecond) {
