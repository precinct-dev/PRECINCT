...
func checkSpireServer(ctx context.Context, cfg Config, deps Deps) ComponentStatus {
	...
	out, err := deps.Exec.Run(ctx, "docker", "compose", "exec", "-T", "spire-server", "/opt/spire/bin/spire-server", "agent", "list")
	if err != nil {
...
func (ExecDockerLister) PS(ctx context.Context) ([]DockerContainer, error) {
	out, err := runCombined(ctx, "docker", "compose", "ps", "--all", "--format", "json")
	if err != nil {
		return nil, err
	}
