...
| Network segmentation | compose-limited | CNI-enforced default-deny `NetworkPolicy` manifests | No CNI policy enforcement in Compose bridge network | Enforce strict gateway-level destination allowlists and run compose segmentation checks |
