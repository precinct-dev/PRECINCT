# OpenClaw Adaptation Guide (Enterprise-Hardened via PRECINCT Gateway)

Status: Draft (architecture-level proposal for a future port/adaptation)

This document describes how to adapt **OpenClaw** (a popular consumer-oriented, multi-channel agentic system) to run inside the **PRECINCT** using the **PRECINCT Gateway**, with **surgery rather than a complete re-engineering**.
...
### In scope
- Mapping OpenClaw’s core surfaces to PRECINCT Gateway planes:
  - LLM egress (“brain”)
  - Context/Memory
  - Tools/Actions
...
4. **All external actions are governed and audited at one authority boundary (PRECINCT Gateway).**
...
- **PRECINCT Gateway becomes the mandatory policy boundary** for:
...
## 4. Mapping OpenClaw To PRECINCT Gateway Planes
...
- Channel traffic MUST be wrapped in the PRECINCT Gateway ingress envelope and submitted to **Ingress Admission** before it can:
...
- Do not make PRECINCT Gateway a universal protocol gateway for Telegram/WhatsApp/etc.
- Instead, deploy **Channel Connectors** as “tool-plane adjacent” services (or separate ingress workers) that:
  ...
  - convert inbound events into the PRECINCT Gateway ingress envelope
  - submit them to PRECINCT Gateway Ingress Admission
...
This keeps PRECINCT Gateway protocol-agnostic while still ensuring consistent, centralized security enforcement.

**Outbound messages**
- OpenClaw responses should not directly call channel APIs from the agent runtime.
- Provide outbound messaging as governed tools (e.g., `send_message(channel=telegram, ...)`) executed via the PRECINCT Gateway Tool Plane against the Channel Connector.
...
- OpenClaw model calls are routed through **PRECINCT Gateway Model Plane**, which enforces:
...
- Prefer an OpenAI-compatible “base URL” redirection (where OpenClaw supports it) so that existing provider client code can point at PRECINCT Gateway without refactoring call sites.
- If OpenClaw uses multiple API shapes (“openai-completions”, “openai-responses”), PRECINCT Gateway should expose compatible endpoints (or provide a tiny compatibility shim).

**Budgets introduce new failure modes**
- When a budget is exhausted, PRECINCT Gateway returns a reason-coded denial.
...
**Target architecture mapping**
- Tool execution should be mediated through the PRECINCT Gateway Tool Plane, including:
...
- Re-expose those capabilities through PRECINCT Gateway-governed tools:
  - `web_fetch` (PRECINCT Gateway tool) rather than OpenClaw’s native `web-fetch`
  - `web_search` (PRECINCT Gateway tool)
  - any file/system actions via tool plane rather than native execution

OpenClaw’s SSRF guard remains useful, but PRECINCT Gateway becomes the central policy point.
...
- Treat “memory retrieval output” as a context contribution:
  - before adding retrieved memory to the prompt, submit it to PRECINCT Gateway Context Admission endpoint
...
**Target architecture mapping**
- PRECINCT Gateway should focus on **immutable, external limits** and evidence:
...
**Key point**
- If every meaningful action (model/tool/context/memory/ingress) is mediated by PRECINCT Gateway, then the loop can remain internal to OpenClaw while still being constrained externally.
...
- PRECINCT Gateway performs token substitution when calling providers.
...
- In Docker Compose: split OpenClaw into a container with no Internet egress; expose only PRECINCT Gateway + connector networks.
...
- Centralized enforcement and evidence at PRECINCT Gateway
...
1. **Force model egress through PRECINCT Gateway**
   - Configure OpenClaw provider base URLs to point to PRECINCT Gateway.
   - Remove dependence on raw env keys; use SPIKE references.

2. **Force tool execution through PRECINCT Gateway**
   - Disable native network tools (`web-fetch`, `web-search`) in the runtime profile.
   - Use PRECINCT Gateway tool plane equivalents instead.

3. **Enforce ingress admission**
   - Run channel connectors (Telegram/WhatsApp/etc) outside the agent runtime.
   - All inbound messages go through PRECINCT Gateway Ingress Admission.

4. **Enforce context/memory admission**
   - Gate any memory retrieval inserted into prompts via PRECINCT Gateway Context Admission.

5. **Apply immutable loop limits**
   - Ensure OpenClaw includes session IDs, agent IDs, and trace IDs in all requests to PRECINCT Gateway so external enforcement is reliable.

## 10. Next Steps (In This Repo)

- Create a dedicated backlog epic for “OpenClaw-in-PRECINCT Gateway” adaptation with:
